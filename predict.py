...
# 预测就是train_fine_tune里面的set_test改一下
def set_test(test_iter, model_file):
    ...
    print("跑完了")
    a = [x for x in pred_label_list]
    dataframe = pd.DataFrame({'label':a})
    dataframe.to_csv(r"/home/wzhang21/NLP/sample_submit.csv",index=False)
    print("结果写入文件了")

    # 预测，有这里的dev.csv是一个有标签的测试集，所以有true_list，可以算一下F1，和之前跑验证集的时候对比一下，如果差不多代码没问题
