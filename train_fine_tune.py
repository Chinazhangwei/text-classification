...
if config_.pretrainning_model == 'nezha':
    ...
    from transformers import BertTokenizer, RobertaConfig, AlbertConfig, BertConfig
...
def list2ts2device(target_list):
    """把utils的数据写入到gpu"""
    target_ts = torch.from_numpy(np.array(torch.LongTensor(target_list)))
    return target_ts.to(device)

# train 大部分不需要改，改utils.py就够了
def train(train_iter, test_iter, config):
    ...
    if config.restore_file is not None: # 训练中断是否读进来，从来没用过
    ...
    # 预训练模型参数，把整个nezha模型拆开，整个模型分为预训练，BIlstm 分类，预训练模型是固定的，别人的，lr小一点，别的权重随机初始化lr大一点
    ...
    for i in range(config.train_epoch):
        ...
        for input_ids, input_mask, segment_ids, cls_list, seq_length in tqdm(train_iter):
            # 转成张量，送到GPU里面，算LOSS，backword反向传播
            ...
            cum_step += 1
        # 验证集验证的分数通过分数保存整体模型名字，每个epoch模型的分数
...
# 当前验证集的评估得分，验证的过程，预测就是set_test这里改一下
...
def softmax(x, axis=1):
   ...
   # 验证utils.py没有问题，训练集训练，验证集用来验证当前的分数，returnn回到train，通过分数（模型的名字）保存，保存每个epoch模型的分数
if __name__ == '__main__':
    ...
    # train包括训练和验证，模型切出来一个训练集和验证集，训练集训练，验证集验证当前的分数，分数return回去train过程中
